...
def main():
	...
	parser.add_option("-r","--genelist",action="store",type="string",dest="gene_list",help="Gene list in bed foramt. All reads hits to exon regions (defined by this gene list) will be spilt into a separate BAM file, the remaining reads will saved into another BAM file.")
	parser.add_option("-o","--out-prefix",action="store",type="string",dest="output_prefix",help="Prefix of output BAM files. \"prefix.in.bam\" file contains reads hit to the gene list specified by \"-r\", \"prefix.ex.bam\" contains reads that cannot mapped gene list. \"prefix.junk.bam\" contains qcfailed or unmapped reads.")
